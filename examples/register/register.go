...
func main() {
...

	{
		res, err := cli.RegisterSymbols(
			context.Background(),
			&kabuspb.RegisterSymbolsRequest{Symbols: []*kabuspb.RegisterSymbol{{Symbol: "9433", Exchange: kabuspb.Exchange_EXCHANGE_TOUSHOU}}})
...

	{
		res, err := cli.RegisterSymbols(
			...
			&kabuspb.RegisterSymbolsRequest{Symbols: []*kabuspb.RegisterSymbol{
				{Symbol: "1320", Exchange: kabuspb.Exchange_EXCHANGE_TOUSHOU},
				{Symbol: "1329", Exchange: kabuspb.Exchange_EXCHANGE_TOUSHOU},
			}})
...

	{
		res, err := cli.RegisterSymbols(
			context.Background(),
			&kabuspb.RegisterSymbolsRequest{Symbols: []*kabuspb.RegisterSymbol{{Symbol: "166060018", Exchange: kabuspb.Exchange_EXCHANGE_ALL_SESSION}}})
